...
all_measured %>% 
  filter(habitat == "marsh" & high_low_carbon == "low") %>%
  ggplot(aes(fraction_organic_matter, fraction_carbon, col = study_id)) +
  geom_point(alpha = 0.5, size = 1.5) +
  theme_bw(base_size = 15)
  # theme(legend.position = "bottom")
ggsave("low_C_marsh.jpg", height = 9, width = 12)
```
...
briandata <- all_measured %>% 
  ...
  # filter(habitat == "marsh" | habitat == "mangrove") %>% 
  select_if(function(x) {!all(is.na(x))}) %>% 
  ...
  rename(carbon_flag = fraction_carbon_flag, 
         carbon_method = fraction_carbon_method, 
         carbon_type = fraction_carbon_type) %>% 
  select(-fraction_organic_matter, -fraction_carbon) %>% 
  select(contains("_id"), percent_organic_matter, percent_carbon, carbon_flag, carbon_method, carbon_type, everything())
...
briandata %>% 
  # filter(study_id == "Burden_et_al_2018") %>%
  ggplot() +
  geom_point(aes(percent_organic_matter, percent_carbon, col = carbon_measured_or_modeled), alpha = 0.6) +
  facet_wrap(~habitat)
