...
library(tidyverse)
input_file01 <- "data/primary_studies/Maxwell_et_al_2023/03_data_format/data/core_level/Smeaton_unpublished_C-SIDE/Physical_geochemical_properties_narrow_cores.csv"
...
input_data02 <- input_data01 %>% 
  ...
  mutate(Source = source_name,
         ...
         Habitat_type = case_when(Marsh_zone == "Mudflat" ~ "Mudflat",
                                  Marsh_zone == "Terrestrial" ~ "Upland",
                                  Marsh_zone %in% c("HM", "LM") ~ "Salt marsh",
                                  # don't know what FW and PM and MM stand for
                                  T ~ NA),
         Country = "UK") %>% 
