...
all <- left_join(ds, cores) %>% 
  ...
  mutate(fraction_carbon = case_when(core_id %in% c("WSC12", "WSC13_2", "WSC10_1", "SH3-5") ~ NA_real_, 
...
all_measured %>% 
  ...
  # filter(study_id == "Breithaupt_et_al_2014") %>%
  ggplot(aes(fraction_organic_matter, dry_bulk_density)) +
  geom_point(alpha = 0.5, size = 0.5) +
  theme_bw(base_size = 15) + 
  facet_wrap(~study_id, scales = "free", ncol = 5) +
  theme(legend.position = "bottom")
```
