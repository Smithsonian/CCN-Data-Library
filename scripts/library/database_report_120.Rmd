...
if(file.exists("data/library_metrics/core_synthesis_history.csv")){
  
  synthesis_history <- read_csv("data/library_metrics/core_synthesis_history.csv")
...
growth_fig <- synthesis_history_trim %>% 
  ...
  theme_bw()
ggsave(growth_fig, filename = "./data/library_metrics/figures/library_growth.jpg")
...
# add most recent update to the table
current_habitat_change <- current_cores %>% count(country, habitat) %>% 
    mutate(version = current_version, date = Sys.Date())
habitat_change <- bind_rows(habitat_change, current_habitat_change)
...
ggsave(plot = hab_plot, filename = "data/library_metrics/figures/library_growth_habitat.jpg")
