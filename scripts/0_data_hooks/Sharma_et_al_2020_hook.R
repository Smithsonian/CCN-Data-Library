...
## Soil core data curation script for "SWAMP Dataset-Mangrove Soil Carbon-Koh Kong–2014-15",
##  Sharma, S.; MacKenzie, R.A.; Tieng, T.; Soben, K.; Tulyasuwan, N.; Resanond, A.; Blate, G.; Litton, C.M.; Apwong, M
## contact: Rose Cheney, cheneyr@si.edu 


## dataset - https://doi.org/10.17528/CIFOR/DATA.00265
## associated publication - https://doi.org/10.1016/j.scitotenv.2019.135416
...
#load in data 
sitedata <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_SiteData.csv")
plot <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_Plot.csv")
disturbance <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_Disturbance.csv")
subplot <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_Subplot.csv")
soil <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_Soil.csv") 
soil_depth <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_Soildepth.csv") 
soilsubp_CN <- read_csv("data/primary_studies/Sharma_et_al_2020/original/KohKong_Soil_2020-10-23_Soilsubp.csv")
...
#core_id = site_id + subplot_id

cores <- plot %>% rename(PLOTID = ID) %>% 
                  mutate(study_id = id,
                         site_id = "Kong Koh",
                         core_id = paste(site_id, ID, sep = "_"),
                         position_method = "other low resolution",
                         position_notes = "plot level coordinates",
...
cores <- subplot %>% mutate(study_id = id,
                         site_id = "Kong Koh",
                         core_id = paste(site_id, ID, sep = "_"),
                         position_method = "other low resolution",
                         position_notes = "plot level coordinates",
...
## ... Impacts ####
impacts <- disturbance %>% mutate(study_id = id,
                                  site_id = "Kong Koh",
                                  core_id = "x",
                                  impact_class = case_when(DISTRUBID == 21 ~ "restored",
                                                           DISTRUBID == 100 ~ "NA",
                                                           DISTRUBID == 22 ~ "disturbance not specified", 
                                                           TRUE ~ "degraded")) %>% 
                            select(-ID, -SUBPID, -DISTURBN, -DISTRUBID,-DISTURBANCE_NOTES, - DYEAR, -CITID)
