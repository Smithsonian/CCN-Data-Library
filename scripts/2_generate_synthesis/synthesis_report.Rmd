...
join_uncontrolled_attributes <- bind_rows(methods_uncontrolled, cores_uncontrolled, ds_uncontrolled) %>% 
  mutate(value = case_when(grepl("[...]", attribute_name) ~ "row_number_id", T ~ value)) %>% 
...
Instances of multiple observations for the same interval within a unique core (and unique method ID) were found in the following `r length(unique(dup_interval$study_id))` studies: `r sort(unique(dup_interval$study_id))`
...
```{r OM-C, echo=FALSE, fig.width=12, fig.height=20, warning=FALSE}
