...
# column_names <- colnames(ccrcn_synthesis$cores)
# valid_columns <- filter(database_structure, table == "cores")$attribute_name
# non_matching_columns <- subset(column_names, !(column_names %in% valid_columns))
#     
# cores_uncontrolled <- ccrcn_synthesis$cores %>% 
#   select(study_id, non_matching_columns)
